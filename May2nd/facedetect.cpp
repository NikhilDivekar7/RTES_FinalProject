...
pthread_attr_getscope(&main_attr, &scope);
    ...
    cv::CommandLineParser parser(argc, argv,
        "{help h||}"
        "{cascade|data/haarcascades/cascade_monster.xml|}"
        "{nested-cascade|data/haarcascades/cascade_redbull.xml|}"
    ...
    thirdCascadeName = "data/haarcascades/banana.xml";
    ...
    sqlite3_stmt *stmt;
    sqlite3_prepare_v2(db, "SELECT PRICE FROM OBJECT_PRICE WHERE OBJECT_ID LIKE 1", -1, &stmt, NULL);
	...
	sqlite3_finalize(stmt);
        ...
        for(;;)
        {
            ...
            detectAndDraw( frame1, cascade, nestedCascade, thirdCascade, scale, tryflip );
    ...
    else
    {
        ...
        if( !image.empty() )
        {
            detectAndDraw( image, cascade, nestedCascade, thirdCascade, scale, tryflip );
            waitKey(0);
        }
        else if( !inputName.empty() )
        {
